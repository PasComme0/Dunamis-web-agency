...
export default function TarifsClient({ hero }: TarifsClientProps) {
  const heroImageUrl = hero?.image_url ?? null;
  const heroOpacity = hero?.opacity ?? 0.35;
  const heroAlt = hero?.alt ?? 'Tarifs hero background';

  return (
    <div className="flex flex-col w-full bg-background">
      {/* HERO SECTION */}
      <section className="relative pt-48 pb-24 px-6 md:px-12 bg-foreground text-background overflow-hidden">
        {heroImageUrl && (
