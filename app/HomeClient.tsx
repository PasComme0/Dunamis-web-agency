...
import { ArrowUpRight, CheckCircle2, ChevronDown, Monitor, Smartphone, PenTool, Lightbulb, ArrowRight, Star, MessageSquare, HelpCircle, Clock, Sparkles } from 'lucide-react';
...
interface HomeClientProps {
  hero: HeroImage | null;
  featuredProjects: Pick<Project, 'id' | 'href'>[];
}

// Fallback featured project hrefs (keyed by a stable identifier)
const FALLBACK_HREFS: Record<string, string> = {
  'aura-botanica': '/work/aura-botanica',
  'nexus-wealth': '/work/nexus-wealth',
};

export default function HomeClient({ hero, featuredProjects }: HomeClientProps) {
  ...
  const heroOpacity = hero?.opacity ?? 0.8;
  const heroAlt = hero?.alt ?? 'Atmospheric architecture';

  // Map featured project hrefs from DB (index 0 = aura-botanica, index 1 = nexus-wealth)
  const project1Href = featuredProjects[0]?.href ?? FALLBACK_HREFS['aura-botanica'];
  const project2Href = featuredProjects[1]?.href ?? FALLBACK_HREFS['nexus-wealth'];
              ...
              <div className="group grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
                <div className="lg:col-span-7 order-2 lg:order-1 overflow-hidden bg-muted">
                  <div className="relative aspect-[4/3] w-full transform transition-transform duration-700 group-hover:scale-105">
                    <Image
                      src="https://picsum.photos/seed/Dunamis-home-feat1/1200/900"
                ...
                <div className="lg:col-span-5 order-1 lg:order-2 flex flex-col justify-center">
                  ...
                  <Link href={project1Href} className="inline-flex items-center gap-2 text-sm font-medium">
            ...
            <Reveal>
              <div className="group grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
                <div className="lg:col-span-5 flex flex-col justify-center">
                  ...
                  <Link href={project2Href} className="inline-flex items-center gap-2 text-sm font-medium">
                    Voir le site <ArrowUpRight className="w-4 h-4" />
                  </Link>
                </div>
                <div className="lg:col-span-7 overflow-hidden bg-muted">
                  <div className="relative aspect-[4/3] w-full transform transition-transform duration-700 group-hover:scale-105">
                    <Image
                      src="https://picsum.photos/seed/Dunamis-home-feat2/1200/900"
